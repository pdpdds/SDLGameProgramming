...
****************************************************************/
{
  ...
  if(CasStream)
  {
    Pos= SDL_RWtell(CasStream);
    if(Pos&7)
      if(SDL_RWseek(CasStream,8-(Pos&7),SEEK_CUR))
      {
        if(Verbose&0x04) puts("FAILED");
        //rewind(CasStream);return;		
		SDL_RWseek(CasStream, 0, SEEK_SET); return;
      }

    while(SDL_RWread(CasStream, Buf,1,8)==8)
    ...
    //rewind(CasStream);
	SDL_RWseek(CasStream, 0, SEEK_SET);
  }
...
****************************************************************/
{
  ...
  if(CasStream)
  {
    //J=fgetc(CasStream);
	  char k = 0;
	SDL_RWread(CasStream, &k, 1, 1);
	J = k;
    //if(J<0) rewind(CasStream);
	if (k<0) SDL_RWseek(CasStream, 0, SEEK_SET);
    else { R->AF.B.h=k;R->AF.B.l&=~C_FLAG; }
  }
...
****************************************************************/
{
  ...
  if(CasStream)
  {
	...
	SDL_RWwrite(CasStream, TapeHeader,1,8);
    R->AF.B.l&=~C_FLAG;
  }   
...
//20180130
	SDL_RWwrite(CasStream, R->AF.B.h, 1, 1);
    R->AF.B.l&=~C_FLAG;
  }
