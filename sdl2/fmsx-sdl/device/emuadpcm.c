...
static e_uint32 clk, rate;

FILE *fp;

EMU8950_ADPCM_API void ADPCM_init(e_uint32 c, e_uint32 r)
{
  clk = c;
  rate = r;
}
...
EMU8950_ADPCM_API void ADPCM_delete(ADPCM *_this)
{
  if(_this)
  {
    free(_this->memory[0]);
...
EMU8950_ADPCM_API void ADPCM_reset(ADPCM *_this)
{
  int i;
...
INLINE static int update_stage(ADPCM *_this)
{
...
INLINE static void update_output(ADPCM *_this, e_uint32 val)
{
  static e_uint32 F[] = 
  {
...
INLINE static e_uint32 calc(ADPCM *_this)
{
  e_uint32 val;
...
EMU8950_ADPCM_API e_int16 ADPCM_calc(ADPCM *_this)
{
  if(_this->reg[0x07]&R07_SP_OFF) return 0;

  return calc(_this);
}

EMU8950_ADPCM_API void ADPCM_writeReg(ADPCM *_this, e_uint32 adr, e_uint32 data)
{
  adr &= 0x1f;
  data &= 0xff;
  ...
  case 0x11: /* DELTA-N (H) */
    _this->reg[adr] = data ;
    _this->delta_n = rate_adjust(((_this->reg[0x11]<<8)|_this->reg[0x10])<<GETA_BITS) ;
    break;
...
EMU8950_ADPCM_API e_uint32 ADPCM_status(ADPCM *_this)
{
  return _this->status;
}
