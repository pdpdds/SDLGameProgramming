...
DynObj::DynObj(const DynObj& obj)
  : BoxObj(obj), mSpeed(0.0)
  , mSquareSpeed(0.0)
  , mSpeedChanged(true)
{
  PrevPos() = obj.PrevPos();
  Pos() = obj.Pos();
  SetVel(obj.VelX(), obj.VelY());
}

void DynObj::Move( double delta_time )
{
  VecXy offset = delta_time * Vel();
  mPrevPos = mPos;
  mPos += offset;
}