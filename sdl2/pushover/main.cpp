...
static std::string getLocaleDir(void)
{
  ...
  return std::string((stat("locale", &st) == 0) ? "locale" : "locale");
#endif
}
...
static unsigned int getKeyMask(void) {
  ...
  if ( keystate[SDL_GetScancodeFromKey(SDLK_UP)] ) keyMask |= KEY_UP;
  if ( keystate[SDL_GetScancodeFromKey(SDLK_DOWN)] ) keyMask |= KEY_DOWN;
  if ( keystate[SDL_GetScancodeFromKey(SDLK_LEFT)] ) keyMask |= KEY_LEFT;
  if ( keystate[SDL_GetScancodeFromKey(SDLK_RIGHT)] ) keyMask |= KEY_RIGHT;
  if ( keystate[SDL_GetScancodeFromKey(SDLK_SPACE)] ) keyMask |= KEY_ACTION;
