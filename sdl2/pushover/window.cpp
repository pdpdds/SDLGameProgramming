...
void helpWindow_c::displayCurrentPage(void)
{
  ...
  if (page == 0)
  {
    ...
    if (getTextHeight(&par, _(help.c_str())) > (unsigned int)par.box.h) {
      par.font = FNT_SMALL;
    }
...
void listWindow_c::redraw(void) {
  ...
  while (true)
  {
    ...
    while (ypos2 + lineH < gr.blockY()*(y+h-1)) {
      ...
      if (line2 == (unsigned int)current && (unsigned int)ypos2 < (unsigned int)gr.blockY()*(y+h/2))
      {
        back = true;
        break;
      }
...
listWindow_c * getTimeoutWindow(surface_c & surf, graphics_c & gr) {
	...
	title = _("Not quite, you were not fast enough");
...
bool InputWindow_c::handleEvent(const SDL_Event & event)
{
  ...
  if (event.type == SDL_KEYDOWN)
  {
    ...
    {
      if (getTextWidth(FNT_NORMAL, input+(char)event.key.keysym.sym) < gr.blockX()*(w-2))
      {
        char utf8[10];
        size_t s = utf8EncodeOne(event.key.keysym.sym, utf8, 10);
