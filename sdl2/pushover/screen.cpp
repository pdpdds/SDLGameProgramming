  ...
  animationState(0), blockX(g.blockX()), blockY(g.blockY())
{
...
#ifdef _WIN32  
	window = SDL_CreateWindow(WINDOW_TITLE, 100, 100, g.resolutionX(), g.resolutionY(), SDL_WINDOW_SHOWN);
#else
  //SDL_Window * window = SDL_CreateWindow("Alien Blaster", 0, 0, 0, 0, 0);
	...
	float ratioX = (float)CCFG::REAL_GAME_WIDTH / (float)CCFG::GAME_WIDTH;
	float ratioY = (float)CCFG::REAL_GAME_HEIGHT / (float)CCFG::GAME_HEIGHT;
	SDL_RenderSetScale(rR, ratioX, ratioY);
#endif

#ifndef _WIN32
	SDL_GetWindowSize(window, &REAL_SCREEN_WIDTH, &REAL_SCREEN_HEIGHT);
#endif
	...
	//SDL_RenderSetIntegerScale(SDL_global_renderer, SDL_TRUE);
	SCREEN_WIDTH = g.resolutionX();
	SCREEN_HEIGHT = g.resolutionY();
	
	video = SDL_CreateRGBSurface(0, g.resolutionX(), g.resolutionY(), 24, 0xFF, 0xFF << 8, 0xFF << 16, 0);

	sdl_texture_ = SDL_CreateTexture(SDL_global_renderer, SDL_PIXELFORMAT_RGB24, SDL_TEXTUREACCESS_STREAMING, g.resolutionX(), g.resolutionY());
...
void screen_c::flipComplete(void)
{
  //SDL_Flip(video);
	...
	//SDL_RenderClear(SDL_global_renderer);
	SDL_UpdateTexture(sdl_texture_, NULL, video->pixels, video->pitch);
	SDL_Rect srcRect;
	SDL_Rect destRect;

	srcRect.x = 0;
	srcRect.y = 0;
	srcRect.w = destRect.w = SCREEN_WIDTH;
	srcRect.h = destRect.h = SCREEN_HEIGHT;
	destRect.x = 0;
	destRect.y = 0;
	...
	SDL_RenderCopyEx(SDL_global_renderer, sdl_texture_, &srcRect, &destRect, 0, 0, SDL_FLIP_NONE);
...
void screen_c::flipDirty(void)
{
  ...
  for (int i = 0; i < numrects; i++)
	  SDL_RenderCopyEx(SDL_global_renderer, sdl_texture_, &rects[i], &rects[i], 0, 0, SDL_FLIP_NONE);
  SDL_RenderPresent(SDL_global_renderer);
  animationState = 0;
}
...
bool screen_c::flipAnimate(void)
{
  ...
  for(int i= 0; i < count; i++)
	  SDL_RenderCopyEx(SDL_global_renderer, sdl_texture_, &rects[i], &rects[i], 0, 0, SDL_FLIP_NONE);
  //SDL_RenderDrawRects(SDL_global_renderer, rects, count);
  SDL_RenderPresent(SDL_global_renderer);
