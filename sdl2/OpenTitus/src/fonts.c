...
*/
    ...
    if ((font->sub[CHAR_SPACE] = SDL_ConvertSurfaceFormat(surface, SDL_GetWindowPixelFormat(pWindow), 0)) == NULL) {
        sprintf(lasterror, "Error: Not enough memory to load fonts!\n");
        freepixelformat(&(pixelformat));
        return (0);
    }
...
SDL_Surface * SDL_LoadChar(unsigned char * fontdata, int offset, SDL_PixelFormat * pixelformat){
	...
	surface2 = SDL_ConvertSurfaceFormat(surface, SDL_GetWindowPixelFormat(pWindow), 0);
