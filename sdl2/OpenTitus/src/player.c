...
extern DISPLAY_SPRITES(TITUS_level *level);
...
#else
    if (keystate[SDL_GetScancodeFromKey(SDLK_F1)] && (RESETLEVEL_FLAG == 0)) { //F1 = suicide
        CASE_DEAD_IM(level);
        RESETLEVEL_FLAG--;
        return 0;
    }
    if (keystate[SDL_GetScancodeFromKey(SDLK_F2)]) { //F2 = game over
        GAMEOVER_FLAG = true;
        return 0;
    }
    if (keystate[SDL_GetScancodeFromKey(SDLK_e)]) { //E = display energy
        BAR_FLAG = 50;
    }
    if (keystate[SDL_GetScancodeFromKey(SDLK_s)] && (devmode == 1)) { //S = skip to next level
        NEWLEVEL_FLAG = true;
    }
    if (keystate[SDL_GetScancodeFromKey(SDLK_F4)]) { //F4 = view status page
...
// TODO: ADD!    SCREEN_3(); //Test for hidden credits screen
    ...
    //Part 2: Determine the player's action, and execute action dependent code
    X_FLAG = keystate[SDL_GetScancodeFromKey(SDLK_LEFT)] | keystate[SDL_GetScancodeFromKey(SDLK_RIGHT)]; //Set if either is true
    Y_FLAG = keystate[SDL_GetScancodeFromKey(SDLK_UP)] | keystate[SDL_GetScancodeFromKey(KEY_JUMP)] | keystate[SDL_GetScancodeFromKey(SDLK_DOWN)]; //Set if either is true
    if (NOCLIP) {
        if (keystate[SDL_GetScancodeFromKey(KEY_LEFT)]) {
            player->sprite.speedX = -100;
        } else if (keystate[SDL_GetScancodeFromKey(KEY_RIGHT)]) {
            player->sprite.speedX = 100;
        } else {
            player->sprite.speedX = 0;
        }
        if (keystate[SDL_GetScancodeFromKey(KEY_UP)] || keystate[SDL_GetScancodeFromKey(KEY_JUMP)]) {
            player->sprite.speedY = -100;
        } else if (keystate[SDL_GetScancodeFromKey(KEY_DOWN)]) {
            player->sprite.speedY = 100;
        } else {
    ...
    if (CHOC_FLAG != 0) {
        ...
        } else if (!PRIER_FLAG && (keystate[SDL_GetScancodeFromKey(KEY_UP)] || keystate[SDL_GetScancodeFromKey(KEY_JUMP)]) && (SAUT_FLAG == 0)) {
        ...
        } else if (PRIER_FLAG || ((SAUT_FLAG != 6) && keystate[SDL_GetScancodeFromKey(KEY_DOWN)])) {
        ...
        if (keystate[SDL_GetScancodeFromKey(KEY_SPACE)] && !PRIER_FLAG) {
            if (!DROP_FLAG) {
    ...
    } else if (keystate[SDL_GetScancodeFromKey(KEY_LEFT)]) {
        newsensX = -1;
    } else if (keystate[SDL_GetScancodeFromKey(KEY_RIGHT)]) {
        newsensX = 1;
    } else if (SENSX == -1) {
...
static int BLOCK_YYPRG(TITUS_level *level, uint8 floor, uint8 floor_above, uint8 tileY, uint8 tileX) {
    ...
    case 6: //Ladder
        ...
        if (floor_above != 6) { //ladder
            ...
            if ((keystate[SDL_GetScancodeFromKey(KEY_UP)] || keystate[SDL_GetScancodeFromKey(KEY_JUMP)]) && (order == 6)) { //action UP + climb ladder
                ARAB_BLOCK_YU(player); //Stop fall
                return 0;
            }
        }
...
*/
    ...
    case 24:
        ...
        DECELERATION(player);
        if (CARRY_FLAG) {
            if (!keystate[SDL_GetScancodeFromKey(KEY_UP)] && !keystate[SDL_GetScancodeFromKey(KEY_JUMP)]) { //Ordinary throw
                speedX = 0x0E * 16;
                speedY = 0;
                if (player->sprite.flipped) {
                    speedX = 0 - speedX;
